...
export default function LoginPage() {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center gap-4 p-6 text-white bg-gradient-to-b from-indigo-950 to-black">
      ...
      <input className="w-full max-w-sm p-3 rounded text-black" placeholder="Email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
      <input className="w-full max-w-sm p-3 rounded text-black" placeholder="Password" type="password" value={pass} onChange={(e) => setPass(e.target.value)} />
      <button onClick={handleAuth} className="px-4 py-2 bg-indigo-600 rounded hover:bg-indigo-500">
      ...
      <Link href="/" className="mt-6 underline">← Back to Ask</Link>
    </main>
  );
}
